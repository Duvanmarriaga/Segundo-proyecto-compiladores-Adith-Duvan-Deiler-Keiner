...
class mostrar():
    ...
    def ventana(self):
        self.vista=Tk()
        self.listpila=list()
        self.vista.title(" Palindromo Impar")
        self.vista.geometry('600x400')
        self.texto=Label(self.vista,text="Ingrese palabra del lenguaje (ab) impar:",font=("Agency FB",14))
        self.texto.pack(padx=0,pady=0)
        self.texto.place(x=10, y=10)
        self.dato=Entry(self.vista,text="validar")
        self.dato.place(x=10, y=40)
        self.boton= Button(self.vista,text="Validar",fg="black",command=self.iniciarAutomata)
    ...
    def validar(self):     
        ...
        if valen == 1 and total >=3 and validar2 is not None:
            ...
            for i in self.lista:
                ...
                cont=cont+1
            for i in lenguaje:
                botonp= Button(self.vista,text=i,fg="white",background="Light Blue",command=self.PruebaDeAnimaciones)
    ...
    def iniciarGrafo(self):
        ...
        self.canvas = Canvas(width=380,height=220,bg="#EFEFEF")
    ...
    def generarPila(self):
        ...
        i=0
        for i in range(n):
                botonp= Button(self.vista,text="_",fg="white",background="Red",command=self.PruebaDeAnimaciones)
                botonp.pack(padx=200,pady=200,ipadx=130,ipady=10)
                botonp.place(x=450, y=(300-pos))
                pos = pos + 25
    ...
    def cambiarLetra(self,i):
        if(self.__indexP==-1):
            self.lista[self.__indexP+1].configure(background="green")
        self.vista.update()
        if(len(self.dato.get())>i):
            self.lista[i].configure(background="green")
    ...
    def OnEstadocirculoQ (self):    
        self.canvas.create_oval(175, 120, 235, 180, width=5, fill='blue')#circulo del arco de transicion de segundo estado
        self.textoEstadoQ()
        self.vista.update()
        time.sleep(0.5)
